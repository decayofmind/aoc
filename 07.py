...
def get_args(p, ip, n, modes, out=True):
    ...
    if out:
        ...
        modes[0] = 1
    args = [
        x if modes[len(modes) - i - 1] == 1 else p[x]
...
for var in combs:
    queues: List[queue.Queue] = [queue.Queue() for x in range(5)]
    queues[0].put(0)
    threads = []
    for n, phase in enumerate(var):
        ...
        if n < 4:
            ...
            q_out = 0
        x = threading.Thread(name='{0}-{1}'.format(phase, n),
                             target=run,
                             args=(p, phase, q_in, q_out))
        threads.append(x)
        x.start()
